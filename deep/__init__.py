...
def train_model_super(X,y,model,num_iter=1000):
    batch_size=10#hyper_params["batch_size"]
...
#    input_var=model.get_input_var()
#    updates=model.get_updates()
#    train_fn = theano.function([input_var,model.target_var], model.loss, updates=updates)
    #train_fn = theano.function([input_var], model.prediction)#, updates=updates)
#    input_dim=3600#(hyper_params["num_input"],)
    #imgs=[img_i.reshape(input_dim) for img_i in imgs]
#    batch,n_batches=tools.get_batch(imgs,batch_size)
#    n_batches=len(y)
#    n_cats=7
...
#            tmp=[int(y[i])]
#            tmp=np.array(tmp)
            #tmp=tmp.astype(theano.config.floatX)
            #loss_i=train_fn(X[i])
            #print(loss_i.dtype)
#            y_dist=tools.to_dist(y[i],n_cats)
#            loss_i=train_fn(X[i], tmp)#y_dist)
#            cost_e.append(loss_i)
#        cost_mean=np.mean(cost_e)
#        print(str(epoch) + " "+str(cost_mean))
#    return model