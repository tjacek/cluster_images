...
def smooth_helper(action_i):
    ...
    smooth_series_i=[series_ij.rolling(window=3)
                        for series_ij in series_i]
    smooth=np.array([series_ij.median().dropna() for series_ij in smooth_series_i])
    n=smooth.shape[1] 
    frames=[smooth[:,i] for i in range(n)]
    return utils.actions.new_action(action_i,frames)

if __name__ == "__main__":
    in_path="../../AA_konf3/united_seqs/"#nn_0"
    out_path="../../AA_konf3/smooth_seqs/"
