...
def deep_feats(in_path,seq_path,out_path,aggregate_type='simple',
              extractor_type='deep',preproc_type='proj',
              dataset_format='mhad_dataset'):
...
def prepare_path(path):
    utils.paths.dirs.make_dir(path)
    return utils.paths.Path(path)

def all_feats(in_path,seq_path,out_path,aggregate_type='dtw',
              extractor_type='deep',preproc_type='proj',dataset_format='mhad_dataset'):
    local_feats(in_path,seq_path,extractor_type,preproc_type,dataset_format)
    gc.collect()
...
def local_feats(in_path,seq_path,extractor_type='deep',preproc_type='proj',dataset_format='mhad_dataset'):
    read_actions=utils.actions.read.ReadActions(dataset_format)
    actions=read_actions(in_path)
    extractor=select_extractor(extractor_type,preproc_type=preproc_type)
...
def select_preproc(preproc_type):
    ...
    raise Exception("No preproc "+ preproc_type)
...
in_path="../exper/proj"
nn_path="../exper/models"
seq_path="../exper/seqs"
out_path="../exper/feats"
deep_feats(in_path,seq_path,out_path,aggregate_type='simple',
            extractor_type=nn_path,preproc_type='proj', dataset_format='mhad_dataset')
